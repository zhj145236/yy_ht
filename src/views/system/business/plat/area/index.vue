  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        <!-- 搜索 -->
        <el-input v-model="query.name" clearable size="small" placeholder="输入区域名称搜索" style="width: 200px;"
    ...
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU"
      ...
      <el-form ref="form" inline :model="form" :rules="rules" size="small" label-width="80px">
        ...
        <el-form-item v-if="form.isTop === '1'" style="margin-bottom: 0;" label="上级区域" prop="pid">
          <treeselect
            v-model="form.pid"
    ...
    >
      ...
      <el-table-column prop="zoneType" label="区域类型">
        <template slot-scope="scope">
          <span>{{ scope.row.zoneType }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column v-permission="['admin','area:edit','area:del']" label="操作" width="130px" align="center"
                       fixed="right">
        <template slot-scope="scope">
          <udOperation
            :data="scope.row"
...
<script>
  ...
  export default {
    ...
    cruds() {
      return CRUD({title: '区域', url: 'api/area', crudMethod: {...crudArea}})
    },
    ...
    methods: {
      getAreaDatas(tree, treeNode, resolve) {
        ...
        setTimeout(() => {
          crudArea.getAreas(params).then(res => {
            resolve(res.content)
          })
        }, 100)
      },
      ...
      [CRUD.HOOK.afterToCU](crud, form) {
        ...
        if (form.id != null) {
          this.getSupAreas(form.id)
        } else {
          this.getAreas()
        }
      },
      getSupAreas(id) {
        crudArea.getAreaSuperior(id).then(res => {
      ...
      getAreas() {
        crudArea.getAreas({enabled: true}).then(res => {
          this.areas = res.content.map(function (obj) {
            if (obj.hasChildren) {
              obj.children = null
            }
      ...
      loadAreas({action, parentNode, callback}) {
        if (action === LOAD_CHILDREN_OPTIONS) {
          crudArea.getAreas({enabled: true, pid: parentNode.id}).then(res => {
      ...
      changeEnabled(data, val) {
        ...
        this.$confirm('此操作将 "' + msg + '" ' + data.name + '区域, 是否继续？', '提示', {
